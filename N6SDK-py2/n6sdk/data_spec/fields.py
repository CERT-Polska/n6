        ...
        for attr_name, obj in per_instance_attrs.items():
            if not hasattr(cls, attr_name):
                raise TypeError(
                    ...
                    .format(cls.__name__, attr_name))                          #3: `__name__`->`__qualname__`
...
class DateTimeField(Field):
    ...
    def clean_result_value(self, value):
        ...
        if isinstance(value, (str, unicode)):                                    #3: `unicode`--
            return self._parse_datetime_string(value)
        raise TypeError(
            '{!r} is neither a str/bytes/bytearray nor a '
...
class UnicodeField(Field):
    ...
    def clean_result_value(self, value):
        ...
        if not isinstance(value, (str, unicode, bytearray)):                     #3: `unicode`->`bytes`
            raise TypeError('{!r} is not a str/bytes/bytearray instance'.format(value))
        value = self._fix_value(value)
        self._validate_value(value)
        return value
...
class IPv6Field(UnicodeField):
    ...
    * when cleaning a result value -- the address is normalized to a
    ...
    def clean_param_value(self, value):
        ...
        return unicode(ipv6_obj.exploded)                                        #3: `unicode`--

    def clean_result_value(self, value):
        ipv6_obj = super(IPv6Field, self).clean_result_value(value)
        return unicode(ipv6_obj.compressed)                                      #3: `unicode`--
    ...
    def _fix_value(self, value):
        ...
        try:
            ...
            if '.' in last_segment and not IPv4_STRICT_DECIMAL_REGEX.search(last_segment):
                raise ValueError('{!r} is not a valid IPv4-like suffix'.format(last_segment))
...
class IPv6NetField(UnicodeField):
    ...
    * when cleaning a result value --
    ...
    def convert_param_prepared_value(self, ipv6_obj, prefixlen):
        ...
        str = unicode                                                            #3--
        ipv6 = str(ipv6_obj.exploded)                                            #3: `str(`-- `)`--
    ...
    def _fix_value(self, value):
        ...
        try:
            ...
            if str(prefixlen) != prefixlen_str:
                raise ValueError('{!r} != {!r}'.format(str(prefixlen), prefixlen_str))
...
class IntegerField(Field):
    ...
    def _check_range(self, value):
        assert isinstance(value, (int, long))                                    #3: `long`--
    ...
    def _do_coerce(self, value):
        ...
        if isinstance(value, (str, bytes, bytearray)):
            ...
            if '.' in value:
                ...
                return int(value)
        elif isinstance(value, (int, long)):                                     #3: `long`--
            return int(value)
        else:
...
class ResultListFieldMixin(Field):
    ...
    * :attr:`sort_result_list` (default value: :obj:`False`; if
    ...
    def clean_result_value(self, value):
        if not is_seq(value):
            raise TypeError('{!r} is not a *non*-str/bytes/bytearray '
...
class DictResultField(Field):
    ...
    *result dictionary* which is cleaned with *data specification's*
    ...
    def _adjust_key(self, key):
        return unicode(verified_as_ascii_str(key))                               #3--
    ...
    def _get_illegal_keys_repr(self, keys):
        if self.key_to_subfield is not None and self.default_subfield is None:
            illegal_keys = keys - self.key_to_subfield.viewkeys()                #3
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(self.must_be_unique, (str, bytes, bytearray)):
            raise TypeError("{}'s 'must_be_unique' is expected to "
                            ...
                            "str/bytes/bytearray (got: {!r})".format(
                                self.__class__.__name__,                          #3: `__name__`->`__qualname__`
                                self.must_be_unique))
