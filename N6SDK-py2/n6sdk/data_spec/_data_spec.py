...
class Ext(dict):
    ...
    def __repr__(self):
        return '{}({})'.format(self.__class__.__name__,                          #3: `__name__`->`__qualname__`
...
class BaseDataSpec(object):
    ...
    def clean_param_dict(self, params,
        ...
        empty_value_list_keys = frozenset(
            key for key, param_values in params.viewitems()                           #3
        ...
        keys = self._clean_keys(
            params.viewkeys() - (empty_value_list_keys | frozenset(ignored_keys)),    #3
            self._all_param_fields.viewkeys() - frozenset(forbidden_keys),            #3
            self._required_param_fields.viewkeys() | frozenset(extra_required_keys),  #3
    ...
    def clean_result_dict(self, result,
        ...
        """
        keys = self._clean_keys(
            result.viewkeys() - frozenset(ignored_keys),                               #3
            self._all_result_fields.viewkeys() - frozenset(forbidden_keys),            #3
            self._required_result_fields.viewkeys() | frozenset(extra_required_keys),  #3
            frozenset(discarded_keys),
    ...
    def param_field_specs(self, which='all', multi=True, single=True, **kwargs):
        ...
        field_items = self.filter_by_which(
            ...
            **kwargs)
        if not multi:
            field_items &= self._single_param_fields.viewitems()                  #3
        if not single:
            field_items -= self._single_param_fields.viewitems()                  #3
    ...
    def filter_by_which(cls, which, all_fields_dict, required_fields_dict):
        ...
        Raises:
            ...
            valid.
        """
        all_fields = all_fields_dict.viewitems()                                 #3
        required_fields = required_fields_dict.viewitems()                       #3
    ...
    def _clean_keys(keys, legal_keys, required_keys, discarded_keys,
        ...
        return {unicode(verified_as_ascii_str(key))                              #3: `unicode`--
    ...
    def _iter_clean_param_items(self, params, keys):
        ...
        for key in keys:
            ...
            if not (isinstance(param_values, list) and all(isinstance(val, (str, unicode))  #3: `unicode`--, just `str`
                                                           for val in param_values)):
                safe_repr_func = ((lambda obj: ascii_str(object.__repr__(obj)))
                                  if field.sensitive
                                  else repr)                                     #3: `repr`->`ascii`
