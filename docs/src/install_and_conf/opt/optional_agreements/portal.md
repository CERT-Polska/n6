...
Introducing any _Agreements_ within an _n6_ instance causes minor changes to the _n6 Portal_'s appearance, allowing users to interact with the agreements, i.e., to agree or disagree (on behalf of the user's organization) to the agreements' terms.

## Registration form

If any agreements exist, they are shown at the bottom of the second page of the _registration form_. Whether they are checked by default or not is determined by the `default_consent` flag (as per [model](model.md)).
...
_Organization agreements page. Here: with a single agreement item. Note that the item is not checked as accepted even though the agreement's flag `default_consent` is set (that flag concerns only the Registration form)._
