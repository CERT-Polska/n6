...
CLASSIFICATION_FIELDS = ['classification.taxonomy',
                         'classification.identifier',
                         'classification.type']
...
class BaseConverter(object):
    ...
    def convert(self, source_data):
        ...
        for source_key, value in data.iteritems():
    ...
    def _unpack_extra_data(self, received_extra_data):
        ...
        for key, value in received_extra_data.iteritems():
...
class N6ToIntelConverter(BaseConverter):
    ...
    extra_field_name_out = 'extra'

    def __init__(self):
        self._mapping = N6_TO_INTEL_MAP
        self._keylist = INTEL_TO_N6_MAP.iterkeys()
    ...
    def _extract_address_fields(self, address):
        for single_addr in address:
            temp_dict = {}
            for source_key, value in single_addr.iteritems():
                destination_key = self._mapping.get(source_key)
...
class IntelToN6Converter(BaseConverter):
    ...
    def _get_n6_category_from_mapping(data):
        for key, val in CLASSIFICATION_MAPPING.iteritems():
