...
class ParserTestMixin(TestCaseMixin):
    ...
    # Please remember to set it in your subclass when testing such
    # blacklist parsers that determine the `_bl-current-time` event
    # attribute from message headers (such as 'mail_time' or
    # 'http_last_modified'...).
    ...
    def test__input_callback(self):
        ...
        for raw_data, expected in self.cases():
            if not isinstance(raw_data, bytes):
                raise TypeError(
                    f'each raw input data body (i.e., the first item of '
                    f'each of 2-tuples defining parser test cases) should '
    ...
    def cases(self):
        ...
        Yields:
            ...
            0) raw input data body (bytes),

            1) one of:

               a list of dicts containing variable items of
               output data bodies expected to be passed (as
               JSON-formatted bytes) into publish_output()
               [here "variable items" means: all items except:
               'id', 'rid', 'source' and parser's `constant_items`]
