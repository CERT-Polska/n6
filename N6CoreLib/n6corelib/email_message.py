...
from n6lib.unpacking_helpers import iter_unzip_from_bytes, gzip_decompress
...
class NoMatchingFileError(ValueError):
    """
    Raised by `ReceivedEmailMessage.get_matching_file_content()`
    if no file matches.
    """


# TODO: doc
# TODO later: upgrade to have `email.message.EmailMessage` as its base
#             with all goodies from modern `email.*` stuff...
class ReceivedEmailMessage(email.message.Message):
    ...
    def iter_filenames_and_contents(self, multifile_unpacking=False):
        if self.is_multipart():
            ...
            # un-ZIP if necessary
                ...
                try:
                    names_and_contents = list(iter_unzip_from_bytes(payload))
