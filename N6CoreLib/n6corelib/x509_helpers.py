...
def get_single_x509_rdn_value(x509_name, nid):
    ...
    Args/kwargs:
        ...
        `nid`:
            ...
            (note: M2Crypto.m2.NID_organizationName == .x509_helpers.ORG;
            M2Crypto.m2.NID_organizationalUnitName == .x509_helpers.OU;
            M2Crypto.m2.NID_commonName == .x509_helpers.CN).

    Returns:
        A single RDN value as a string (e.g. 'cert.pl' for
        nid=.x509_helpers.ORG or 'kowalski@example.com' for
        nid=.x509_helpers.CN...).
