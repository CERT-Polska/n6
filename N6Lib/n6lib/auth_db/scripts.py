...
class _BaseAuthDBScript(SQLAuthDBConfigMixin):
    ...
    def __init__(self, quiet=False, **kwargs):
        self.quiet = quiet
        super(_BaseAuthDBScript, self).__init__(**kwargs)
    ...
    def configured_secondary_engine(self):
        engine = self.make_db_engine(url_overwrite_attrs={'database': None})
        try:
            yield engine
        finally:
            engine.dispose()
    ...
    @contextlib.contextmanager
    def configured_db_session(self):
        if not _BaseAuthDBScript.__db_session_already_configured:
            db_session.configure(bind=self.engine)
            _BaseAuthDBScript.__db_session_already_configured = True
        db_session.rollback()
        try:
            yield
        except:
    ...
    __db_session_already_configured = False
...
class _DropDatabaseMixin(object):
    ...
    def drop_db_if_exists(self, secondary_engine):
        self.msg('Dropping auth database if it exists...')
        quoted_db_name = self.quote_sql_identifier(self.db_name)
        secondary_engine.execute('DROP DATABASE IF EXISTS {}'.format(quoted_db_name))
...
class CreateAndInitializeAuthDB(_DropDatabaseMixin, _BaseAuthDBScript):
    ...
    def run(self):
        with self.configured_secondary_engine() as secondary_engine:
            if self._drop_db_if_exists:
                self.drop_db_if_exists(secondary_engine)
            self.create_db(secondary_engine)
        self.create_tables()
        with self.configured_db_session():
            self.insert_criteria_categories()

    def create_db(self, secondary_engine):
    ...
    def insert_criteria_categories(self):
        ...
        for category in CATEGORY_ENUMS:
            ...
            db_session.add(CriteriaCategory(category=category))
...
class PopulateAuthDB(_BaseAuthDBScript):
    ...
    """
    Populate the Auth DB with some basic data:
    ...
    * a new organization (and a new user associated with it - you will
      be asked for the password, unless you use option -P) with granted
    ...
    * n6 event categories, if not already added.
    ...
    def make_argument_parser(cls):
        ...
        parser.add_argument('-n', '--no-password', action='store_true',
                            help=('resign from setting the password for the '
                                  'added user (note that the password can be '
                                  'set later using the Admin Panel)'))
        ...
        parser.add_argument('-S', '--sources', metavar='SOURCE',
                            ...
                            help=('data source identifiers for whom general access subsources '
                                  ...
                                  'defaults are: {} (note that by specifying this option you '
                                  'override them)'.format(' '.join(cls.DEFAULT_SOURCES))))
    ...
    def parse_arguments(cls, parser):
        ...
        if arguments.pop('no_password'):
            arguments['password'] = None
        else:
            arguments['password'] = getpass.getpass(
                'Please, type in the password for the new user who '
                'will be identified by organization id "{org_id}" '
                'and user login "{login}": '.format(**arguments))
        return arguments
