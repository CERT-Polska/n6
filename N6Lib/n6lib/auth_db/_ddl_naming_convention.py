...
def optional_suffix_with_abbrev_constraint_name(constraint, _table):
    ...
    assert not isinstance(name, conv), (
        'SQLAlchemy machinery should have ensured that {!a} is *not* '
        'an instance of {!a} -- but it is!'.format(name, conv))
...
def _get_constraint_columns(constraint, table):
    ...
    if not columns:
        raise NotImplementedError(
            'unsupported case: constraint {!a} seems '
...
def _verify_name_is_supported(name):
    if not isinstance(name, str):
        raise NotImplementedError(
            'unsupported case: name {!a} is not a `str`'.format(name))
    if not name:
        raise ValueError("'name is empty (that's strange!)'")
    unsupported_characters = set(name) - _SUPPORTED_NAME_CHARACTERS
    if unsupported_characters:
        raise NotImplementedError(
            'name {!a} contains the following unsupported characters: {}'.format(
                name,
                ', '.join(map(ascii, sorted(unsupported_characters)))))
