...
import sys                                                                       #3--
try:                                                                             #3--
    from email.generator import BytesGenerator       #3: dedent this and the following lines...
    ...
    assert sys.version_info[0] < 3                                               #3--
    from email.generator import Generator as BytesGenerator                      #3--
    from email.header import Header                                              #3--
    from email.message import Message as EmailMessage                            #3--
    from email.utils import (                                                    #3--
        formataddr,                                                              #3--
        getaddresses,                                                            #3--
        mktime_tz,                                                               #3--
        parsedate_tz,                                                            #3--
...
if sys.version_info[0] < 3:                                                      #3--
    UsualHeaderRaw = Union[String, Header]                                       #3--
    AddressHeaderRaw = Union[String, Iterable[String], Header]                   #3--
    DateHeaderRaw = Union[datetime.datetime, String, Header]                     #3--
HeaderRaw = Union[
    UsualHeaderRaw,
    AddressHeaderRaw,
    DateHeaderRaw,
...
class _MessageHelpersMixin(object):
    ...
    def copy_message(self, message):
        ...
        if sys.version_info[0] < 3:                                              #3--
            return copy.deepcopy(message)                                        #3--
    ...
    def add_multiple_headers(self, message, headers):
        ...
        assert isinstance(message, EmailMessage)
        assert sys.version_info[0] < 3 or isinstance(message.policy, EmailPolicy)   #3: `sys.version_info[0] < 3 or `--
    ...
    def prepare_header_value(self, header_name, value):
        ...
        if not isinstance(value, str):
            raise TypeError('{!r} is not a `str`'.format(value))
        if header_name.lower() == 'subject':
            self.validate_subject_value(value)
        return value

    def prepare_header_value(self, header_name, value):                          #3: remove whole method definition (keeping the above one)
        # type: (String, HeaderRaw) -> Header
        ...
        if header_name.lower() == 'subject':
            self.validate_subject_value(value)
        header = Header(as_unicode(value),
                        header_name=header_name,
                        maxlinelen=78)
        return header

    def validate_subject_value(self, value):
        if isinstance(value, Header):                                            #3--
            return                                                               #3--
        if value is None:
        ...
        if len(value) > SUBJECT_MAX_LENGTH:
            raise ValueError(
                ...
                'characters; got: {} characters ({!r})'.format(
                    SUBJECT_MAX_LENGTH,
                    len(value),
                    value))
    ...
    def _adjust_addr_value(self, value):
        if not value:
            raise ValueError('no e-mail address given (got: {!r})'.format(value))
        str = basestring                                                           #3--
...
class MailSendingAPI(ConfigMixin, _MessageHelpersMixin):
    ...
    def __init__(self, settings=None):
        ...
        self._smtp_client_deposit = ThreadLocalContextDeposit(repr_token=self.__class__.__name__)    #3: `__name__` -> `__qualname__`
    ...
    def send_message(self,
        ...
        if sys.version_info[0] >= 3 and not isinstance(message.policy, EmailPolicy):                 #3: `sys.version_info[0] >= 3 and `--
        ...
        if recipient_problems:
            ...
            LOGGER.warning('Unable to send e-mail to some recipient(s): %r', recipient_problems)
    ...
    def _addr_items_from_header_values(self, addr_header_values):
        if sys.version_info[0] < 3:                                              #3--
            raw_addrs = [self._header_value_as_ascii_str(header_value)           #3--
                         for header_value in addr_header_values]                 #3--
            return list(getaddresses(raw_addrs))                                 #3--
        return [addr_item
                for header_value in addr_header_values
                    for addr_item in header_value.addresses]

    def _header_value_as_ascii_str(self, header_value):                          #3: remove whole method definition
        if header_value is None:
            raise ValueError('no header value given (got: {!r})'.format(header_value))
        try:
            return unicode(header_value).encode('ascii')
        except (UnicodeError, LookupError) as exc:  # Header.__unicode__() may raise LookupError
            for_repr = (header_value.encode() if isinstance(header_value, Header)
                        else header_value)
    ...
    def _pure_addr(self, addr_item):                                             #3: remove whole method definition (keeping the above one)
        _, addr = addr_item
        if not EMAIL_OVERRESTRICTED_SIMPLE_REGEX.search(addr):
            raise ValueError(
                'e-mail address {!r} does not match '
    ...
    def _verify_no_unsupported_headers(self, message):
        for header_name in UNSUPPORTED_HEADER_NAMES:
            ...
            if values:
                raise NotImplementedError('header {!r} is not supported by {} '
                                          '(got: {!r})'.format(header_name,
    ...
    def _get_current_smtp_client(self):
        ...
        if client is None:
            raise RuntimeError(
                ...
                'interface)'.format(self.__class__.__name__))                                #3: `__name__` -> `__qualname__`
        return client
    ...
    def _flatten_message(self, message, _):                                                  #3: remove whole method definition (keeping the above one)
        bytes_io = BytesIO()
...
class MailMessageBuilder(_MessageHelpersMixin):
    ...
    def __setattr__(self, name, value):
        if name not in self.PUBLIC_SETTABLE_ATTRS:
            raise AttributeError('setting attribute {!r} is illegal'.format(name))
        if name in {'misc_headers', 'render_context'}:
            value = dict(value) if value is not None else {}
        elif sys.version_info[0] < 3:                                            #3--
            pass                                                                 #3--
        elif name == 'policy':
    ...
    def _adjust_policy(self, policy):
        if policy is None:
            ...
            raise TypeError('unsupported type of `policy`: '
                            '{!r}'.format(type(policy)))
        return policy

    def _make_message_obj(self):
        return EmailMessage(policy=self.policy)

    def _make_message_obj(self):                                                 #3: remove whole method definition (keeping the above one)
        return EmailMessage()
    ...
    def _ready_body(self):
        ...
        if body is None:
            raise RuntimeError('{!r}.body has not been set (without it '
                               'no message can be built)'.format(self))
        if isinstance(body, bytes):                                              #3--
            return body.decode('utf-8')                                          #3--
        str = unicode                                                            #3--
        if isinstance(body, str):
            return body
        # In the future we may want to support multipart messages --
        # but see the comment in `_verify_content_type_supported()`.
        raise NotImplementedError('{!r}.body is of an unsupported '
                                  'type: {!r} (the value is: {!r})'
    ...
    def _set_body(self, message, body):                                          #3: remove whole method definition (keeping the above one)
