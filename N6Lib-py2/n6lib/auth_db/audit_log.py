...
from n6lib.class_helpers import properly_negate_eq                               #3--
...
class _ModelInstanceWrapper(object):
    ...
    def __repr__(self):
        return '{class_name}({model_instance!r})'.format(
            class_name=self.__class__.__name__,                                  #3: `__name__`->`__qualname__`
    ...
    __ne__ = properly_negate_eq                                                  #3--
                                                                                 #3--
    # Because we have the equality test implemented (see:  `__eq__()`)           #3--
    # and we don't need the hashability feature, let's be on a safe              #3--
    # side and have this feature explicitly disabled.                            #3--
    __hash__ = None                                                              #3--
    ...
    def _make_jsonable_from_attr_value(self, name, value):
        ...
        if isinstance(value, (str, bool, int, float)):
            return value
        if isinstance(value, long): return value                                 #3--
...
class _AuditLogEntryBuilder(DictWithSomeHooks):
    ...
    assert OPERATION_TO_REQUIRED_FINAL_KEYS.viewkeys() == VALID_OPERATIONS       #3: `viewkeys`->`keys`
