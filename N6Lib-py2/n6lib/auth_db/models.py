...
import datetime
import string
from collections import MutableSequence             #3: -> `from collections.abc import MutableSequence`
...
from n6lib.common_helpers import (
    formattable_as_str,                                                          #3-- oraz usunąć wszystkie użycia tej funkcji
    make_exc_ascii_str,
    make_hex_id,
    with_dunder_unicode_from_str,                                                #3--
)
...
def ascii_alphanum_and_underscore_only(text):
    ...
    if isinstance(text, str) and _ASCII_ALPHANUM_AND_UNDERSCORE_ONLY_CHARS.issuperset(text):
        return text
    raise AssertionError('{!r} is not an ASCII-alphanumeric-and-underscore-only string')
...
class AuthDBCustomDeclarativeMeta(DeclarativeMeta):
    ...
    def __init__(cls, *args, **kwargs):
        ...
        with_dunder_unicode_from_str(cls)                                        #3--
    ...
    def __verify_assumptions(cls):
        ...
        if not (isinstance(mapper, Mapper) and
            ...
            raise NotImplementedError(
                'For the {!r} model class, some of our basic '
        ...
        for attr_name, column in mapper.columns.items():
            ...
            if not (isinstance(attr_name, str) and
                ...
                raise NotImplementedError(
                    'For the {!r} model class and its attribute {!r}, '
            ...
            if attr_name != column.name:
                raise NotImplementedError(
                    'The property key (model attribute name) {!r} is '
                    '*not* equal to the corresponding database column '
                    'name {!r}. SQLAlchemy supports such cases but, '
...
class _PassEncryptMixin(object):
    ...
    def verify_password(self, password):
        ...
        try:
            ...
            LOGGER.error(
                '%r: could not verify password with bcrypt.verify() '
        ...
        if pending_requests_found:
            try:
                ...
                raise ValueError(
                    ...
                    'config update requests for {!r} found: {!r}.'.format(
                        self,
...
def sanity_checks_after_user_provisional_mfa_config_insert_or_update(mapper, connection, target):
    ...
    if ids != {my_id}:
        ...
        raise AssertionError(
            'trying to link {!r} to a WebToken to whom other provisional '
    ...
    if token_type != WEB_TOKEN_TYPE_FOR_MFA_CONFIG:
        raise AssertionError(
            'trying to link {!r} to a WebToken whose `token_type` is '
            'not equal to {!r}! (found `token_type`: {!r})'.format(
                target,
                WEB_TOKEN_TYPE_FOR_MFA_CONFIG,
                token_type))
