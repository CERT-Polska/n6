...
class RandomEvent(ConfigMixin):
    ...
    * By adding the attribute's name to the `port_values` list in
    ...
    def _int_to_ip(int_ip):
        ...
        # TODO: the same can be expressed in a simpler way by using int_ip.to_bytes...
        return socket.inet_ntoa(bytes.fromhex(hex(int_ip)[2:].zfill(8)))

    @staticmethod
    def _url_to_domain(url):
        parsed_url = urllib.parse.urlparse(url)
        return as_unicode(parsed_url.hostname)                                   #3: `as_unicode(`-- `)`--
