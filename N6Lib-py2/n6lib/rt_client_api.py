...
class RTClientAPI(ConfigMixin):
    ...
    def _conv_ticket_kind_to_fields_render_spec(cls, opt_value):
        ...
        raw_items = sorted(raw_dict.items())                                     #3: `sorted(`-- `)`--
        for ticket_kind, field_to_template in raw_items:
            ticket_kind = ticket_kind.decode('utf-8')                            #3--
            str = unicode                                                        #3--
            ...
            try:
                ...
                raise ValueError(
                    ...
                    'renderer for ticket kind {!r} - {}'.format(
                        ticket_kind,
    ...
    def new_ticket(self, ticket_kind, data_dict):
        ...
        try:
            ...
            if not isinstance(ticket_id, int):
                ...
                raise TypeError('ticket_id={!r} is not an int'.format(ticket_id))
            if ticket_id < 1:
                # (the `rt` library's API changed or what?!)
                raise ValueError('ticket_id={!r} is not a positive number'.format(ticket_id))
        except:
    ...
    def _prepare_new_ticket_fields(self, renderer, data_dict):
        ...
        LOGGER.debug('Prepared fields for a new ticket: %r', field_name_to_value)
...
class _TicketFieldsRenderer(object):
    ...
    def __init__(self, field_to_template):
        ...
        self._field_names = sorted(field_to_template)                            #3: `sorted` -> `list`
    ...
    def __gen_cleaned_field_to_template_items(self, raw_dict):
        if not isinstance(raw_dict, dict):
            raise TypeError('not a dict: {!r}'.format(raw_dict))
        if _QUEUE_RT_FIELD_NAME not in raw_dict:
            raise ValueError('required RT field name {!r} is missing'
                             .format(_QUEUE_RT_FIELD_NAME))
        raw_items = sorted(raw_dict.items())                                     #3: `sorted(`-- `)`--
        for field_name, template_string in raw_items:
            field_name = field_name.decode('utf-8')                              #3--
            assert isinstance(field_name, unicode)                               #3--
            str = basestring                                                     #3--
            assert isinstance(field_name, str)
            if not isinstance(template_string, str):
                raise TypeError('non-`str` object as RT field value '
                                'template (for field {!r}): {!r}'
    ...
    def generate_rendered_field_items(self, data_dict):
        ...
        for field_name in self._field_names:
            ...
            if sys.version_info[0] < 3:                                          #3--
                yield field_name.encode('utf-8'), field_value.encode('utf-8')    #3--
                continue                                                         #3--
