...
class FalseIfOwnerClassNameMatchesRegex(object):
    ...
    def __init__(self, regex):
        ...
        if not hasattr(regex, 'search') or not hasattr(regex, 'match'):
            raise TypeError('{!r} does not look like a regex object')
        self.__regex = regex
...
def attr_repr(*attr_names):
    ...
    <A x=1, y='qwerty'>
    """
    format_repr = ('<{0.__class__.__name__} ' +                                  #3: `__name__` -> `__qualname__`
...
def get_class_name(instance_or_class):
    ...
    return (
        ...
        if isinstance(instance_or_class, (type, types.ClassType))                #3: <- `types.ClassType`--
