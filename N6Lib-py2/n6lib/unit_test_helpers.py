...
def _patching_method(method_name, patcher_maker, target_autocompletion=True):
    ...
    ***
    ...
    # be in kwargs)...
    def a_patching_method(*args, **kwargs):                                        #3: `*args, **kwargs` -> `self, target, /, *args, **kwargs`
        if len(args) < 2:                                                        #3--
            # we say "1 argument" because we count the second positional         #3--
            # argument (`target`) but not the first one (`self`)                 #3--
            raise TypeError(                                                     #3--
                '{}() takes at least 1 argument '                                #3--
                '(0 given)'.format(method_name))                                 #3--
        self, target = args[:2]                                                  #3--
        if target_autocompletion:
            target = _complete_target(self, target)
        patcher_args = (target,) + args[2:]                                        #3: `[2:]`--
...
class TestCaseMixin(SDKTestCaseMixin):
    ...
    def patch_with_plug(self,
                        ...
                        exc_msg_pattern=('use of {target} is unsupported when running '
                                         'tests from {self.__class__.__name__}'),        #3: `__name__` -> `__qualname__`
                        **patch_kwargs):
...
class ExceptionRaisingPlug(object):
    ...
    def __make_plug_meth(meth_name):
        def plug_method(*args, **_):                                               #3: `*args, **_` -> `self, /, *_, **__`
            self = args[0]                                                       #3--
...
class _ExpectedObjectPlaceholder(object):
    ...
    def __new__(cls, *args, **kwargs):                                           #3: `cls,` -> `cls, /,`
...
class AnyInstanceOf(_ExpectedObjectPlaceholder):
    ...
    >>> any_str_or_integral                                                      #3: `<type 'str'>` -> `<class 'str'>`
    ...
    >>> any_str_or_integral == u'foo'                                            #3: u' -> b'
    False
    >>> u'foo' == any_str_or_integral                                            #3: u' -> b'
    False
...
class AnyDictIncluding(_ExpectedObjectPlaceholder):
    ...
    def __init__(*args, **required_items):                                         #3: `*args,` -> `self, /,`
        [self] = args                                                            #3--
...
class AnyInstanceOfWhoseVarsInclude(AnyInstanceOf, AnyObjectWhoseVarsInclude):
    ...
    def __init__(*self_and_classes, **required_items):                             #3: `*self_and_classes,` -> `self, /, *classes,`
        self = self_and_classes[0]                                               #3--
        classes = self_and_classes[1:]                                           #3--
...
class JSONWhoseContentIsEqualTo(_ExpectedObjectPlaceholder):
    ...
    #>>> json1 == bytearray(b'{"key": 42}')                                      #3: uncomment these lines...
    #True
