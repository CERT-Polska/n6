    ...
    *not* handled by the particular class are passed up the
        ...
        if not isinstance(value, type_spec):
            if error_message is None:
                if isinstance(type_spec, tuple):
                    error_message = 'unexpected type of {!r}'.format(value)
                else:
                    safe_class_name = ascii_str(type_spec.__name__)                      #3: `__name__` -> `__qualname__`
                    error_message = 'unexpected type of {!r} (should be a {})'.format(
                        value,
                        safe_class_name)
    ...
    def verify_in(value, container, error_message=None):
        ...
        if value not in container:
            if error_message is None:
                error_message = '{!r} is not in {!r}'.format(value, container)
            raise DataConversionError(error_message)
        ...
        for input_name, input_value in data:
            if not isinstance(input_name, str):
                raise DataConversionError(
                    'unexpected non-string key ({!r}) in '
        ...
        if collection is not None:
            if value_cell_factory is not None:
                raise TypeError(
                    ...
                    "(got: collection={!r}; value_cell_factory={!r}".format(
...
class NameValuePairConverter(BaseNameValuePairConverter):
    ...
    def __init__(self,
        ...
        if output_name is not None and not isinstance(output_name, str):
            raise TypeError('{!r} is not a string'.format(output_name))
...
class StringValueConverter(BaseConverter):
    ...
    def __call__(self, value):
        ...
        self.verify_isinstance(
            value, str,
            error_message='{!r} is not a string'.format(value))
...
class FlagValueConverter(BaseConverter):
        ...
        if isinstance(raw_arg, str):
            ...
            if non_str_values:
                raise TypeError(
                    ...
                    'the argument: {kind}_flag_strings={raw_arg!r})'.format(
                        non_str_values=', '.join(map(repr, non_str_values)),
                        kind=kind,
                        raw_arg=raw_arg))
        if sys.version_info[0] < 3:                                              #3--
            flags = [as_unicode(f).encode('utf-8') for f in flags]               #3--
    ...
    def __call__(self, value):
        ...
        self.verify_isinstance(
            value, (str, bool),
            error_message='{!r} is neither a string nor a Boolean value'.format(value))
        if isinstance(value, str):
    ...
    def _convert_flag_string(self, value):
        ...
        word = value.lower()
        if sys.version_info[0] < 3:                                              #3--
            word = word.encode('utf-8')                                          #3--
        if not self._true_flag_strings and not self._false_flag_strings:
            raise DataConversionError(
                '{!r} is an illegal value (for this item, strings '
        ...
        if word in self._true_flag_strings:
            ...
            raise DataConversionError(
                '{!r} is an illegal value (for this item these are the only '
                'strings that can be converted to Boolean values: {})'.format(
                    word,
                    ', '.join(
                        sorted(map(repr, self._true_flag_strings)) +
                        sorted(map(repr, self._false_flag_strings)))))
        return word
