...
class MailNoticesAPI(ConfigMixin):
    ...
    config_spec = r'''
        ...
        # section `[jinja_template_based_renderer]`).                            #3: remove the `u` prefix from string literals in the following dict:
    ...
    def _log_exc(self, notice_key, context_entered, important_done, exc_suppressed=False):
        ...
        if important_done:
            LOGGER.warning(
                msg_preamble + (
                    'occurred *after* dealing with dispatch of %r '
            ...
            LOGGER.error(
                msg_preamble + (
                    'occurred *while* dealing with dispatch of %r mail '
            ...
            LOGGER.error(
                msg_preamble + (
                    'occurred *before* dealing with dispatch of %r mail '
...
class _ErrorLoggingMailNoticesDispatcher(_DummyMailNoticesDispatcher):
    ...
    def __call__(self, to, *args, **kwargs):
        LOGGER.error(
            ...
            'causes that a %r mail notice cannot be sent [to: %r]',
            self._notice_key, to)
...
class _ActualMailNoticesDispatcher(MailNoticesDispatcher):
        ...
        if mail_components is None:
            if config['default_lang'] == lang:
                raise ValueError(
                    ...
                    'key {!r} and the language {!r} (which is also '
            ...
            if mail_components is None:
                raise ValueError(
                    'no mail components configuration for the notice '
                    'key {!r} -- neither for the language {!r} nor '
                    'for the default language {!r}'.format(
                        notice_key,
                        lang,
                        config['default_lang']))
    ...
    def _log_smtp_recipients_exc(self, recipient_problems):
        recipients_str = ', '.join(map(repr, sorted(recipient_problems.keys())))
        exc_descr = make_exc_ascii_str()
        LOGGER.error(
            'Suppressing a mail-recipients-related exception which '
            'signals that a %r mail notice could not be sent to '
    ...
    def _log_other_smtp_exc(self, to):
        ...
        LOGGER.error(
            ...
            'caused that a %r mail notice could not be sent [to: %r] '
    ...
    def _log_non_exc_summaries(self, ok_recipients, recipient_problems):
        if recipient_problems:
            err_recipients_str = ', '.join(map(repr, sorted(recipient_problems.keys())))
            LOGGER.warning(
                'A %r mail notice could not be sent to *some* '
                'of the given recipients -- namely to: %s (%r)',
                self._notice_key, err_recipients_str, recipient_problems)
        if ok_recipients:
            ok_recipients_str = ', '.join(map(repr, sorted(ok_recipients)))
            LOGGER.info(
                'A %r mail notice has been sent to: %s',
...
def _adjust_lang(value):
    ...
    if len(value) == 2 and set(value).issubset(string.ascii_uppercase):
        ...
        raise DataConversionError('{!r} is not a valid two-letter '
...
def _adjust_single_mail_component(value):
    ...
    if not isinstance(value, str):
        raise DataConversionError('{!r} is not a str'.format(value))
