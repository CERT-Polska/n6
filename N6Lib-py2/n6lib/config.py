...
class Config(DictWithSomeHooks):
    ...
    **Important proviso**: the *logging configuration* options are read
    ...
    @classmethod
    def make(*args, **kwargs):                                                   #3: `*args` -> `cls, /, *args`
        """
        ...
        Traceback (most recent call last):
          ...
        TypeError: key Decimal('42') is not a str instance
        ...
        super(Config, self).__init__(*args[1:], **kwargs)                        #3: `*args[1:]` -> `*args`
    ...
    def _convert_settings_dict(self, settings):
        ...
        for key, value in settings.items():
            if isinstance(key, unicode):                                         #3--
                key = key.encode('utf-8')                                        #3--
            elif not isinstance(key, str):                                       #3: `elif`->`if`
                LOGGER.warning('Ignoring non-`str` settings key %r', key)
                continue
            if isinstance(value, unicode):                                       #3--
                value = value.encode('utf-8')                                    #3--
            elif not isinstance(value, str):                                     #3: `elif`->`if`
                if not self._is_standard_pyramid_key_of_non_str(key):  # <- to reduce log noise
                    LOGGER.warning(
                        'Coercing non-`str` value %r (of setting %s) '
    ...
    def _make_config_sections(self,
        ...
        for sect_spec in all_sect_specs:
            ...
            free_opt_names = sorted(
                input_opt_dict.viewkeys() -                                      #3 `viewkeys()`->`keys()`
    ...
    def _load_n6_config_files(cls):
        ...
        for sect_name in config_parser.sections():
            ...
            if sys.version_info[0] < 3:                                                          #3--
                assert (                                                                         #3--
                    isinstance(sect_name, basestring) and                                        #3--
                    all(isinstance(opt_name, basestring) and isinstance(opt_value, basestring)   #3--
                        for opt_name, opt_value in opt_name_to_value.items()))                   #3--
                sect_name = as_unicode(sect_name).encode('utf-8')                                #3--
                opt_name_to_value = {                                                            #3--
                    as_unicode(opt_name).encode('utf-8'): as_unicode(opt_value).encode('utf-8')  #3--
                    for opt_name, opt_value in opt_name_to_value.items()}                        #3--
            assert (
...
class ConfigMixin(object):
    ...
    * get_config_full() succeeds but the obtained Config is empty.
    ...
    def __get_config_required(self):
        ...
        if isinstance(config_required, basestring):                              #3--
            config_required = (str(config_required),)                            #3--
