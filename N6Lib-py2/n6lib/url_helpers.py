...
# Copyright (c) 2019-2021 NASK. All rights reserved.
                                                                                   #3: add `b` prefixes to `bytes` literals and doctests in the whole module
                                                                                   #3: remove `u` prefixes from `str` literals and doctests in the whole module
import re
...
DOMAIN_LABEL_SEPARATOR_REGEX = re.compile(u'[.\u3002\uff0e\uff61]')
...
def make_provisional_url_search_key(url_orig):
    ...
    if not isinstance(url_orig, (str, bytes, bytearray)):
        raise TypeError('{!r} is neither `str` nor `bytes`/`bytearray`'.format(url_orig))
    if not url_orig:
...
def _get_ipv6_addr(match):
    ...
    try:
        ...
        ipv6_suffix = as_unicode(ipv6_suffix, 'surrogatepass')                      #3--
...
def _get_port(match, scheme):
    ...
    if (port is None
          or port == conv(':')
          or port == conv(':{}'.format(URL_SCHEME_TO_DEFAULT_PORT.get(scheme)))):
        port = conv('')
    return port
