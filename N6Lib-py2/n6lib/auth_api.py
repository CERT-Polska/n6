...
assert set(AccessZone.__values__) == ACCESS_ZONES                         #3: `__values__` -> `__args__`
assert set(EventDataResourceId.__values__) == EVENT_DATA_RESOURCE_IDS     #3: `__values__` -> `__args__`
            ...
            if url_pattern is not None:
                ...
                try:
                    try:
                        ...
                        match1 = re.compile(url_pattern, re.UNICODE).search    #3: `, re.UNICODE`--
                    except re.error:
        ...
        for org_id, org in org_id_to_node.items():
            ...
            for user_id, user in user_id_to_node.items():
                user_is_blocked = self._is_flag_enabled(
                    user,
                    caption='the user {0!r}'.format(user_id),
                    attribute='n6blocked')
                if user_is_blocked:
                    continue
        ...
        for org_id, subsource_refint, access_zone in sorted(grouped_set):  # (deterministic order)
            ...
            assert (
                (
                    ...
                    az_to_org_ids.viewkeys() == ACCESS_ZONES and                     #3 `.viewkeys()` -> `.keys()`
