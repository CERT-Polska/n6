...
def early_Formatter_class_monkeypatching():  # called in n6lib/__init__.py
    ...
    @functools.wraps(logging.Formatter.formatTime.__func__)                      #3: `.__func__`--
    ...
    if sys.version_info[0] < 3:                                                  #3--
        logging.Formatter.default_time_format = '%Y-%m-%d %H:%M:%S'              #3--
        logging.Formatter.default_msec_format = '%s,%03d'                        #3--
...
class AMQPHandler(logging.Handler):
    ...
    LOGRECORD_EXTRA_ATTRS = {
        ...
        'py_ucs4': (sys.maxunicode > 0xffff),                                    #3--
    ...
    def _make_record_serializer(self):
        ...
        cur_window_cell = [None]  # using 1-item list as a cell for a writable non-local variable      #3: remove this line and uncomment the next one
        #cur_window = None
        loggername_to_window_and_msg_to_count = defaultdict(
            lambda: (cur_window_cell[0], defaultdict(                                                  #3: `cur_window_cell[0]` -> `cur_window`
        ...
        def _should_publish(record):
            ...
            #nonlocal cur_window                                                                       #3: uncomment this line
            loggername = record.name
            msg = record.msg
            cur_window_cell[0] = cur_window = record.created // msg_count_window                       #3: `cur_window_cell[0] = `--
...
class _LogRecordCuttingProxy(object):
    ...
    def __repr__(self):
        return ('{cls.__name__}'                                                 #3: `__name__` -> `__qualname__`
                '({record!r},'
                ' {msg_length_limit!r},'
