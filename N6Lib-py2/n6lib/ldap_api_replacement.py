...
class LdapAPI(SQLAuthDBConfigMixin):
    ...
    def _coerce_search_res_attr_value(self, value):
        ...
        if isinstance(value, str):
            ...
            #assert False, 'should not happen'    # XXX: remove this execution branch     #3: uncomment this line, keep the XXX comment
            return value.decode('utf-8')
        elif isinstance(value, bool):
            return ('TRUE' if value else 'FALSE')
        elif isinstance(value, int):
            return str(value)
        elif isinstance(value, long):                                            #3--
            return str(value)                                                    #3--
            ...
            raise NotImplementedError('cannot coerce {!r} to a `str` '
                                      '(unsupported class: {})'.format(value,
                                                                       get_class_name(value)))

    def _make_dn(self, rdn_type, *unescaped_rdn_values, **kwargs):               #3: `, **kwargs` -> `parent=None`
        parent = kwargs.pop('parent', None)                                      #3--
        if kwargs:                                                               #3--
            raise TypeError('unexpected keyword arguments: {}'                   #3--
                            .format(', '.join(map(repr, sorted(kwargs)))))       #3--
    ...
    def _normalize_attrs(dn, attrs):
        for attr_name, value_list in list(attrs.items()):
            ...
            str = basestring                                                     #3--
            assert all(isinstance(s, str) for s in value_list)
            ready_value_list = ldap_attr_normalizer.get_ready_value_list(dn, attr_name, value_list)
            assert all(isinstance(s, str) for s in ready_value_list)
            if ready_value_list:
...
def get_value_list(node, attr_name):
    ...
    raise TypeError(
        ...
        'or `bytearray` (got: {!r})'.format(value_list))
    ...
    * there is no value and no `default` was specified.
    ...
    try:
        ...
        if default is __sentinel:
            raise ValueError('attribute {!r} has no value'.format(attr_name))    #3: add `from None`
        else:
            return default
