...
def datetime_with_tz_to_utc(dt,
    ...
    ***
    ...
    >>> from dateutil.tz import gettz, enfold
    ...
    ***
    ...
    Traceback (most recent call last):
      ...
    ProblematicTimeValueError: datetime.datetime(2020, 10, 25, 2, 38) is ambiguous in time zone...
    ...
    ***
    ...
    # [Py3] maybe TODO in all doctests and in the following paragraph: get rid of `enfold`...
    #                  (as Py3.6+'s datetime supports `.fold` natively)

    Note that, when dealing with *ambiguous* times, if `dt` (the given
    `datetime.datetime` object) has the attribute `fold=1` (which has
    been introduced in Python 3.6, but can be made available for older
    Python versions by employing the `dateutil.tz.enfold()` helper) it
    can be taken into account in computation of the result -- but *only*
    if the `on_ambiguous_time` argument is set to
    `ReactionToProblematicTime.RELY_ON_OBJECTS`:
    ...
    ...     enfold(mk_dt(2020, 10, 25, 2, 38)), 'Europe/Warsaw',  # results for dt *with* fold=1
    ...     on_ambiguous_time=ReactionToProblematicTime.RELY_ON_OBJECTS)
    datetime.datetime(2020, 10, 25, 1, 38)
    >>> datetime_with_tz_to_utc(
    ...     enfold(mk_dt(2020, 10, 25, 2, 38, tzinfo=gettz('Europe/Warsaw'))),
    ...     on_ambiguous_time=ReactionToProblematicTime.RELY_ON_OBJECTS)
    datetime.datetime(2020, 10, 25, 1, 38)
    ...
    >>> datetime_with_tz_to_utc(
    ...     enfold(mk_dt(2020, 10, 25, 2, 38)), 'Europe/Warsaw',
    ...     on_ambiguous_time=ReactionToProblematicTime.PICK_THE_LATER)
    datetime.datetime(2020, 10, 25, 1, 38)
    >>> datetime_with_tz_to_utc(
    ...     enfold(mk_dt(2020, 10, 25, 2, 38)), 'Europe/Warsaw',
    ...
    Traceback (most recent call last):
      ...
    ProblematicTimeValueError: ...is ambiguous in time zone...
    ...
    >>> datetime_with_tz_to_utc(
    ...     enfold(mk_dt(2020, 10, 25, 2, 38)), 'UTC',
    ...     on_ambiguous_time=ReactionToProblematicTime.RELY_ON_OBJECTS)
    datetime.datetime(2020, 10, 25, 2, 38)
    >>> datetime_with_tz_to_utc(
    ...     enfold(mk_dt(2020, 10, 25, 2, 38)), 'UTC',
    ...
    ***
    ...
    Traceback (most recent call last):
      ...
    ProblematicTimeValueError: datetime.datetime(2020, 3, 29, 2, 45) does not exist in time zone...
    ...
    ***
    ...
    ...     enfold(mk_dt(2020, 3, 29, 2, 45)), 'Europe/Warsaw',              # *with* fold=1 is
    ...
    ***
    ...
    if tz is None:
        if dt.tzinfo is None:
            raise ValueError('no time zone information given: tz is None '
                             '*and* dt={!r} is a "naive" datetime object '
        ...
        if dt.tzinfo is not None:
            raise ValueError('too much time zone information given: tz is not '
                             'None *and* dt={!r} is an "aware" datetime object '
                             '(i.e., with tzinfo *not* being None)'.format(dt))

        if isinstance(tz, str):
            got_tz = gettz(tz)
            if got_tz is None:
                raise LookupError('{!r} is *not* a known time zone name'.format(tz))
            tz = got_tz
    ...
    for reaction_arg_name, reaction_arg in [('on_ambiguous_time', on_ambiguous_time),
        ...
        if not isinstance(reaction_arg, ReactionToProblematicTime):
            raise TypeError('{}={!r} is not a {} enum instance'.format(
                reaction_arg_name,
                reaction_arg,
