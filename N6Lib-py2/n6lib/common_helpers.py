...
# (For more information -- see the docstrings below...)
                                                                                 ...
                                                                                 #3: everywhere in this module: remove u from string literals and doctest results
from __future__ import print_function                                            #3--
from builtins import hex                                                         #3--
from builtins import map                                                         #3--
from builtins import zip                                                         #3--
...
try:                                                                             #3--
    ...
    from _thread import get_ident as get_current_thread_ident                    #3--
from typing import (
    Iterable,
    Iterator,
    List,
...
class PlainNamespace(object):
    ...
    def __init__(*args, **kwargs):                                               #3: `*args,` -> `self, /,`
        try:                                                                   #3--
            # to avoid arg name clash ('self' may be in kwargs)...             #3--
            [self] = args                                                      #3--
        except ValueError:                                                     #3--
            args_length = len(args)                                            #3--
            assert args_length >= 2                                            #3--
            raise TypeError(                                                   #3--
                '{}.__init__() takes no positional arguments ({} given)'       #3--
                .format(type(args[0]).__name__, args_length - 1))              #3--
    ...
    def __repr__(self):
        ...
        return "{.__name__}({})".format(type(self), ", ".join(items))            #3: `__name__` -> `__qualname__`
    ...
    * the constructor takes *only* one optional keyword argument:
    ...
    def __repr__(self):
        ...
        return '<{} object as visible from thread {!r}: {}>'.format(
            type(self).__name__,                                                 #3: `__name__` -> `__qualname__`
...
class NonBlockingLockWrapper(object):
    ...
    def _make_lock_ascii_description(self, lock_description):
        if lock_description is None:
            lock_description = repr(self.lock)
    ...
    * there are additional methods:
    ...
    def _instance_mock():
        ...
        def enter_side_effect():
            return FilePagedSequence.__enter__.__func__(m)                       #3: `.__func__`--

        def exit_side_effect(*args):
            return FilePagedSequence.__exit__.__func__(m, *args)                 #3: `.__func__`--
...
class DictWithSomeHooks(dict):
    ...
    * Instances of the class support copying: both shallow copying (do
    ...
    def __init__(*args, **kwargs):                                               #3: `*args,` -> `self, /, *args,`
        self = args[0]  # to avoid arg name clash ('self' may be in kwargs)...   #3--
    ...
    def fromkeys(cls, *args, **kwargs):
        raise NotImplementedError(
            ...
            'for the {.__name__} class', cls)                                    #3: `__name__` -> `__qualname__`
    ...
    def __repr__(self):
        ...
        return '{.__class__.__name__}({})'.format(self, constructor_args_repr)    #3: `__name__` -> `__qualname__`
...
class NormalizedDict(collections_abc.MutableMapping):
    ...
    def __repr__(self):
        return '{0.__class__.__name__}({1!r})'.format(self, dict(self.items()))  #3: `__name__` -> `__qualname__`
    ...
    """Deprecated -- to be removed when switching to Py3."""                     #3--
    iterkeys = __iter__                                                          #3--
                                                                                 #3--
    def itervalues(self):                                                        #3--
        """Deprecated -- to be removed when switching to Py3."""                 #3--
        return map(operator.itemgetter(1), self._mapping.values())               #3--
                                                                                 #3--
    def iteritems(self):                                                         #3--
        """Deprecated -- to be removed when switching to Py3."""                 #3--
        return iter(self._mapping.values())                                      #3--
                                                                                 #3--
    def keys(self):                                                              #3--
        return collections_abc.KeysView(self)                                    #3--
                                                                                 #3--
    def values(self):                                                            #3--
        return collections_abc.ValuesView(self)                                  #3--
                                                                                 #3--
    def items(self):                                                             #3--
        return collections_abc.ItemsView(self)                                   #3--
                                                                                 #3--
    def copy(self):
...
class LimitedDict(collections.OrderedDict):
    ...
    True
    >>> lo.update([('c', 3), ('d', 4)])                                #3:  `.update(` -> ` |= `  plus  `)`--
    >>> lo == OrderedDict([('a', 1), ('c', 3), ('d', 4)])
    True
    ...
    def __init__(*args, **kwargs):                                                 #3: `*args, **kwargs` -> `self, /, *args, maxlen, **kwargs`
        #self._maxlen = maxlen                                                     #3: uncomment this line
        self = args[0]  # to avoid arg name clash ('self' may be in kwargs)...   #3--
        args = args[1:]                                                          #3--
        try:                                                                     #3--
            self._maxlen = kwargs.pop('maxlen')                                  #3--
        except KeyError:                                                         #3--
            raise TypeError('{0.__class__.__name__}.__init__ needs '             #3--
                            'keyword-only argument maxlen'.format(self))         #3--
    ...
    @classmethod
    def fromkeys(cls, iterable, value=None, **kwargs):                           #3: `**kwargs` -> `*, maxlen`
        """
        ...
        """
        try:                                                                     #3--
            maxlen = kwargs.pop('maxlen')                                        #3--
        except KeyError:                                                         #3--
            raise TypeError('{0.__name__}.fromkeys needs '                       #3--
                            'keyword-only argument maxlen'.format(cls))          #3--
...
class _CacheKey(object):
    ...
    def __repr__(self):
        return '{0.__class__.__name__}{0.args!r}'.format(self)                   #3: `__name__` -> `__qualname__`
...
def make_dict_delta(dict1, dict2):
    ...
    ...     {u'a': 1, u'b': 2.0, 'c': 3, 'd': 4.0},
    ...     {'b': 2, u'c': 3.0, 'd': 42, 'e': 555})
    >>> delta == {
    ...     DictDeltaKey('-', u'a'): 1,
    ...     DictDeltaKey('-', 'd'): 4.0,
    ...     DictDeltaKey('+', 'd'): 42,
...
def picklable(func_or_class):
    ...
    ...     for obj in [a, b, b2, b3, C]:                                                     #3--
    ...         assert not hasattr(obj, '__qualname__')                                       #3--
    ...         obj.__qualname__ = '_make_nontoplevel.<locals>.' + obj.__name__               #3--
    ...     picklable.__qualname__ = 'picklable'                                              #3--
    ...                                                                                       #3--
...
def reduce_indent(s):
    ...
    >>> reduce_indent(u'')
    u''
    >>> reduce_indent(b' ')                                                      #3: add b to the result repr
    ''
    >>> reduce_indent(u'\n')
    u'\n'
    >>> reduce_indent(b'\r\n')                                                   #3: add b to the result repr
    '\n'
    >>> reduce_indent(u'\n \n')
    u'\n\n'
    >>> reduce_indent(b' \r \r\n ')                                              #3: add b to the result repr
    ...
    >>> reduce_indent(u'x')
    u'x'
    >>> reduce_indent(b' x')                                                     #3: add b to the result repr
    'x'
    >>> reduce_indent(u'\nx\n')
    u'\nx\n'
    >>> reduce_indent(b' \r x\r\n ')                                             #3: add b to the result repr
    ...
    if issubclass(tp, (bytes, bytearray)):
        ...
        raise TypeError('{!r} is neither a `str` nor a `bytes`/`bytearray`'.format(s))
...
def splitlines_asc(s, keepends=False):
    ...
    ***
    ...
    >>> s = u"abc\ndef\rghi\vjkl\f\u2028mno\r\npqr\n"
    >>> splitlines_asc(s)
    [u'abc', u'def', u'ghi\x0bjkl\x0c\u2028mno', u'pqr']
    >>> splitlines_asc(s, True)
    [u'abc\n', u'def\r', u'ghi\x0bjkl\x0c\u2028mno\r\n', u'pqr\n']

    ...the results are *different* than when using the method:

    >>> s.splitlines()
    [u'abc', u'def', u'ghi', u'jkl', u'', u'mno', u'pqr']
    >>> s.splitlines(True)
    [u'abc\n', u'def\r', u'ghi\x0b', u'jkl\x0c', u'\u2028', u'mno\r\n', u'pqr\n']

    ***

    For `bytes`/`bytearray`:                                                     #3: everywhere here: add `b` to literals

    >>> b = b"abc\ndef\rghi\vjkl\fmno\r\npqr\n"
    >>> splitlines_asc(b)
    ['abc', 'def', 'ghi\x0bjkl\x0cmno', 'pqr']
    >>> splitlines_asc(b, True)
    ['abc\n', 'def\r', 'ghi\x0bjkl\x0cmno\r\n', 'pqr\n']
    >>> splitlines_asc(bytearray(b'ghi\vjkl\rspam'))
    [bytearray(b'ghi\x0bjkl'), bytearray(b'spam')]

    ...the results are *the same* as when using the method:                      #3: everywhere here: add `b` to literals
    ...
    if isinstance(s, str):
        return [b.decode('utf-8')                                                #3: add 'surrogatepass'...
                for b in s.encode('utf-8').splitlines(keepends)]                 #3: add 'surrogatepass'...
    raise TypeError('{!r} is neither a `str` nor a `bytes`/`bytearray`'.format(s))
...
def limit_str(s, char_limit, cut_indicator='[...]', middle_cut=False):
    ...
    >>> limit_str('Ala ma kota', 10)
    'Ala m[...]'
    >>> limit_str(b'Ala ma kota', 10)                                            #3--
    'Ala m[...]'
    >>> limit_str(u'Alą mą ĸóŧą', 10)
    u'Al\\u0105 m[...]'
    >>> limit_str('Ala ma kota', 11)
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 1000000)
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 9)
    'Ala [...]'
    >>> limit_str('Ala ma kota', 8)
    'Ala[...]'
    >>> limit_str('Ala ma kota', 7)
    'Al[...]'
    >>> limit_str('Ala ma kota', 6)
    'A[...]'
    >>> limit_str('Ala ma kota', 5)
    '[...]'
    >>> limit_str('Ala ma kota', 4)                             # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...

    >>> limit_str('Ala ma kota', 10, middle_cut=True)
    'Ala[...]ta'
    >>> limit_str(b'Ala ma kota', 10, middle_cut=True)                           #3--
    'Ala[...]ta'
    >>> limit_str(u'Alą mą ĸóŧą', 10, middle_cut=True)
    u'Al\\u0105[...]\\u0167\\u0105'
    >>> limit_str('Ala ma kota', 11, middle_cut=True)
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 1000000, middle_cut=True)
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 9, middle_cut=True)
    'Al[...]ta'
    >>> limit_str('Ala ma kota', 8, middle_cut=True)
    'Al[...]a'
    >>> limit_str('Ala ma kota', 7, middle_cut=True)
    'A[...]a'
    >>> limit_str('Ala ma kota', 6, middle_cut=True)
    'A[...]'
    >>> limit_str('Ala ma kota', 5, middle_cut=True)
    '[...]'
    >>> limit_str('Ala ma kota', 4, middle_cut=True)            # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...

    >>> limit_str('Ala ma kota', 12, cut_indicator='****')
    'Ala ma kota'
    >>> limit_str(u'Alą mą ĸóŧą', 12, cut_indicator='****', middle_cut=True)
    u'Al\\u0105 m\\u0105 \\u0138\\xf3\\u0167\\u0105'
    >>> limit_str(u'Alą mą ĸóŧą', 11, cut_indicator='****')
    u'Al\\u0105 m\\u0105 \\u0138\\xf3\\u0167\\u0105'
    >>> limit_str('Ala ma kota', 11, cut_indicator='****', middle_cut=True)
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 11, cut_indicator=u'****')
    'Ala ma kota'
    >>> limit_str('Ala ma kota', 10, cut_indicator='****', middle_cut=True)
    'Ala****ota'
    >>> limit_str(u'Alą mą ĸóŧą', 10, cut_indicator='****')
    u'Al\\u0105 m\\u0105****'
    >>> limit_str('Ala ma kota', 10, cut_indicator=u'****', middle_cut=True)
    'Ala****ota'
    >>> limit_str(u'Alą mą ĸóŧą', 10, cut_indicator=u'****')
    u'Al\\u0105 m\\u0105****'
    >>> limit_str('Ala ma kota', 6, cut_indicator='****', middle_cut=True)
    'A****a'
    >>> limit_str('Ala ma kota', 6, cut_indicator=u'****')
    'Al****'
    >>> limit_str('Ala ma kota', 6, cut_indicator=u'****', middle_cut=True)
    'A****a'
    >>> limit_str('Ala ma kota', 5, cut_indicator='****')
    'A****'
    >>> limit_str('Ala ma kota', 5, cut_indicator=u'****', middle_cut=True)
    'A****'
    >>> limit_str('Ala ma kota', 4, cut_indicator=u'****')
    '****'
    >>> limit_str('Ala ma kota', 4, cut_indicator='****', middle_cut=True)
    '****'
    >>> limit_str(u'Alą mą ĸóŧą', 4, cut_indicator='****')
    u'****'
    >>> limit_str(u'Alą mą ĸóŧą', 4, cut_indicator=u'****', middle_cut=True)
    u'****'
    >>> limit_str('Ala ma kota', 4, cut_indicator='**ą*')       # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    >>> limit_str('Ala ma kota', 4, cut_indicator=u'**ą*',
    ...              middle_cut=True)                           # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    >>> limit_str(u'Alą mą ĸóŧą', 4, cut_indicator='**ą*')
    u'**\\u0105*'
    >>> limit_str(u'Alą mą ĸóŧą', 4, cut_indicator=u'**ą*', middle_cut=True)
    u'**\\u0105*'
    >>> limit_str('Ala ma kota', 3, cut_indicator='****')       # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    >>> limit_str(u'Alą mą ĸóŧą', 3, cut_indicator=u'****',
    ...              middle_cut=True)                           # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...

    >>> limit_str(u'Ala', 4, cut_indicator='')
    u'Ala'
    >>> limit_str('Ala', 4, cut_indicator='', middle_cut=True)
    'Ala'
    >>> limit_str('Ala', 3, cut_indicator=u'')
    'Ala'
    >>> limit_str(u'Ala', 3, cut_indicator=u'', middle_cut=True)
    u'Ala'
    >>> limit_str(u'Ala', 2, cut_indicator='')
    u'Al'
    >>> limit_str('Ala', 2, cut_indicator=u'', middle_cut=True)
    'Aa'
    >>> limit_str('Ala', 1, cut_indicator='')
    'A'
    >>> limit_str('Ala', 1, cut_indicator='', middle_cut=True)
    'A'
    >>> limit_str(u'Ala', 0, cut_indicator='')
    u''
    >>> limit_str('Ala', 0, cut_indicator='', middle_cut=True)
    ''
    >>> limit_str(u'Ala', 0, cut_indicator='*')                 # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    >>> limit_str('Ala', 0, cut_indicator='*',
    ...              middle_cut=True)                           # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    ...
    >>> limit_str('', 10)
    ''
    >>> limit_str(u'', 10, middle_cut=True)
    u''
    >>> limit_str('', 0, cut_indicator='', middle_cut=True)
    ''
    >>> limit_str(u'', 0, cut_indicator='')
    u''
    >>> limit_str('', 0, cut_indicator='*')                     # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    ValueError: ...
    >>> limit_str('', 0, cut_indicator=u'')
    ''
    """
    if not isinstance(s, (str, unicode)):                                        #3: `unicode`--
        raise TypeError('{!r} is not a `str`'.format(s))
    if not isinstance(cut_indicator, (str, unicode)):                            #3: `unicode`--
        raise TypeError('cut_indicator={!r} is not a `str`'.format(cut_indicator))
    if isinstance(s, str) and not isinstance(cut_indicator, str):                #3--
        cut_indicator = cut_indicator.encode('utf-8')                            #3--
    elif isinstance(s, unicode) and not isinstance(cut_indicator, unicode):      #3--
        cut_indicator = cut_indicator.decode('utf-8')                            #3--
    effective_limit = char_limit - len(cut_indicator)
    if effective_limit < 0:
        raise ValueError(
            '`char_limit` is too small: {}, i.e., smaller than '
            'the length of `cut_indicator` ({!r})'.format(
                char_limit,
                cut_indicator))
    if len(s) > char_limit:
...
def as_bytes(obj, encode_error_handling='surrogatepass'):
    ...
    TypeError: ...
    """
    if sys.version_info[0] < 3 and encode_error_handling == 'surrogatepass':     #3--
        encode_error_handling = 'strict'                                         #3--
    if isinstance(obj, memoryview):                                              #3--
        return obj.tobytes()                                                     #3--
    dunder_bytes = getattr(type(obj), '__bytes__', None)                         #3--
    if dunder_bytes is not None:                                                 #3--
        return dunder_bytes(obj)                                                 #3--
    str = unicode                                                                #3--
    ...
    if (isinstance(obj, (bytes, bytearray, memoryview))
        ...
        return bytes(obj)
    raise TypeError('{!r} cannot be converted to bytes'.format(obj))


def formattable_as_str(s):                                                       #3: remove whole definition
    r"""
    **Deprecated -- to be removed when switching to Py3.**
...
def with_dunder_unicode_from_str(cls):                                           #3: remove whole definition
    r"""
    **Deprecated -- to be removed when switching to Py3.**
...
def ipv4_to_int(ipv4, accept_no_dot=False):
    ...
    try:
        str = basestring                                                       #3--
        if isinstance(ipv4, (int, long)):                                        #3: `long`--
            int_value = ipv4
        elif not isinstance(ipv4, str):
            raise TypeError('{!r} is neither `str` nor `int`'.format(ipv4))
...
def iter_grouped_by_attr(collection_of_objects,   # type: Iterable[T]
    ...
    #>>> grouped = iter_grouped_by_attr([d, c, b], 'mu', presort=True)                               #3: uncomment
...
def open_file(file, mode='r', **open_kwargs):
    ...
    *Additional feature:* if `mode` does *not* contain the 'b' marker  <- Py3-only feature      #3: `<- Py3-only feature`--
    ...
    """
    if 'b' not in mode and sys.version_info[0] >= 3:                                            #3: `and sys.version_info[0] >= 3`--
...
def make_hex_id(length=96, additional_salt=b''):
    ...
    >>> isinstance(h, str) and is_hex(h) and len(h) == 96
    True
    >>> h = make_hex_id(additional_salt=u'some salt')
    >>> isinstance(h, str) and is_hex(h) and len(h) == 96
    True
    >>> h = make_hex_id(96)
    >>> isinstance(h, str) and is_hex(h) and len(h) == 96
    ...
    if not 1 <= length <= 96:
        raise ValueError(
            '`length` must be in the range 1..96 ({0!r} given)'.format(length))
    try:
        additional_salt = as_bytes(additional_salt)
    except TypeError:
        raise TypeError(
            '`additional_salt` cannot be converted to bytes (its '
            'type is {})'.format(type(additional_salt).__name__))                #3: `__name__`->`__qualname__`, add `from None`
...
def normalize_hex_id(hex_id, min_digit_num=0):
    ...
    TypeError: ...
    """
    if not isinstance(hex_id, (str, unicode)):                                           #3: `unicode`--
        raise TypeError('`hex_id` must be str, not {}'.format(type(hex_id).__name__))    #3: `__name__`->`__qualname__`
...
def make_exc_ascii_str(exc=None):
    ...
    # Note: to be consistent with standard error displays
    # we use the exc type's `__name__`, not `__qualname__`.
    if type(exc) is types.InstanceType:  # Py2-only (old-style-class-based exception)  #3--
        return '{}: {}'.format(ascii_str(exc.__class__.__name__), ascii_str(exc))      #3--
...
class AtomicallySavedFile(object):
    ...
    *Additional feature:* if `mode` does *not* contain the 'b' marker  <- Py3-only feature      #3: `<- Py3-only feature`--
    ...
    def _adjust_kwargs(self,
        ...
        if 'b' not in mode and encoding is NOT_GIVEN and sys.version_info[0] >= 3:              #3: `and sys.version_info[0] >= 3`--
            encoding = 'utf-8'
        kwargs = {}
        if buffering is not NOT_GIVEN:
            kwargs['buffering'] = buffering
        if encoding is not NOT_GIVEN:
            kwargs['encoding'] = buffering
        if errors is not NOT_GIVEN:
            kwargs['errors'] = buffering
        if newline is not NOT_GIVEN:
            kwargs['newline'] = buffering
        return kwargs
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        if exc_type is not None:
            LOGGER.warning("Exception occurred when trying to save the %r file atomically "
