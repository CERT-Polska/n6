...
def _api_property(api_name):
    ...
    def getter(self):
        ...
        if api is None:
            raise RuntimeError(
                'the {!r} view cannot make use of `{}` because it was '
    ...
    if hasattr(getter, '__qualname__'):                                          #3--
        # (this will be relevant after migration to Python 3.x)                  #3--
        getter.__qualname__ = '{}.{}'.format(_APIS_MIXIN_CLASS_NAME, api_name)       #3: dedent this line
        ...
        with self.mail_notices_api.dispatcher(notice_key,
            ...
            for user_login in login_seq:
                try:
                    ...
                    LOGGER.error('Cannot send a mail notice to the user %r '
...
assert (EssentialAPIsViewMixin.__name__ == _APIS_MIXIN_CLASS_NAME
        and (not hasattr(EssentialAPIsViewMixin, '__qualname__')                 #3: <- remove *this* cond...
             or EssentialAPIsViewMixin.__qualname__ == _APIS_MIXIN_CLASS_NAME))  #3: <- remove `or` keeping *this* cond
...
class WithDataSpecsViewMixin(object):
    ...
    def make_data_spec(cls, **name_to_field):
        ...
        if hasattr(cls, '__qualname__'):  # <- will be relevant after migration to PY3      #3--
            data_spec_class.__qualname__ = '{}.{}'.format(cls.__qualname__,                    #3: dedent (and maybe also join?) these two lines
...
class WithDataSpecsAlsoForRequestParamsViewMixin(WithDataSpecsViewMixin):
    ...
    def _provide_clean_request_params_method(cls):
        ...
        if hasattr(cls, '__qualname__'):  # <- will be relevant after migration to PY3               #3--
            _clean_request_params.__qualname__ = '{}.{}'.format(cls.__qualname__,                        #3: dedent these two lines
