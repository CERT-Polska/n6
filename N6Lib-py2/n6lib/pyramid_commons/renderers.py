...
class _BaseSnortSuricataRenderer(BaseStreamRenderer):
    ...
    def _url_to_url_no_fqdn(self, url):
        if not url:
            return ""
        parsed = urllib.parse.urlparse(as_unicode(url))                          #3: `as_unicode(`-- `)`--
        output = [as_unicode(parsed.path)]                                       #3: `as_unicode(`-- `)`--
        if parsed.query:
            output.extend([u'?', as_unicode(parsed.query)])                      #3: `as_unicode(`-- `)`--
        if parsed.fragment:
            output.extend([u'#', as_unicode(parsed.fragment)])                   #3: `as_unicode(`-- `)`--
...
class StreamRenderer_csv(BaseStreamRenderer):
    ...
    def _dict_to_csv_ready(self, value):
        ...
        if target is not None:
            if isinstance(target, unicode):                                      #3--
                target = target.encode('utf-8')                                  #3--
            details.append("target {}".format(target))
        serialized["details"] = " ".join(details)
        serialized = {                                                           #3--
            k: (v.encode('utf-8') if isinstance(v, unicode)                      #3--
                else v)                                                          #3--
            for k, v in serialized.items()}                                      #3--
        return serialized
