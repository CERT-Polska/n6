...
class Future(object):
    ...
    * *cancelled* -- entered when the future is `cancel()`-ed
    ...
    def __repr__(self):
        return ('<{0.__class__.__name__}:'                                       #3: `__name__` -> `__qualname__`
                ' {0.state_label},'
    ...
    def result(self):
        ...
        with self._cond_var:
            ...
            if self._exc_info is not None:
                raise_(self._exc_info[0], self._exc_info[1], self._exc_info[2])
            return self._res
    ...
    def _set_result(self, res):
        ...
        if just_set:
            LOGGER.debug('%r got result value: %r', self, res)
...
class Task(threading.Thread):
    ...
    def __init__(self, target, args=(), kwargs=None,                             #3: `self,` -> `self, *,`
                 future=None,
    ...
    def cancel_and_join(self, suppress_exc_from_cancel=False):                   #3: `self,` -> `self, *,`
        """
    ...
    def run(self):
        LOGGER.debug('Started: %r (with %r)', self, self._future)
        try:
            if self._initial_sleep is not None:
                time.sleep(self._initial_sleep)
            LOGGER.debug('Starting actual operation of %r...', self)
            self._actual_operation()
        except:
            LOGGER.error('Finishes abruptly: %r (with %r)', self, self._future, exc_info=True)
            raise
        else:
            LOGGER.debug('Finishes normally: %r (with %r)', self, self._future)
    ...
    def _modify_daemon_flag_if_necessary(self,
        ...
        if cancel_and_join_at_python_exit:
            if force_daemon or force_daemon is None:
                ...
                raise ValueError(
                    'the `cancel_and_join_at_python_exit` argument is {!r} '
                    'so the `force_daemon` argument should not be {!r}'.format(
                        cancel_and_join_at_python_exit,
                        force_daemon))
...
class LoopedTask(Task):
    ...
    def __init__(self, loop_iteration_hook, **task_kwargs):                      #3: `self,` -> `self, *,`
        """
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    LOGGER.warning(
                        '%r: suppressing exception got from '
